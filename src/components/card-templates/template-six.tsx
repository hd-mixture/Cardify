...
const TemplateSix = React.forwardRef<HTMLDivElement, TemplateProps>(
  ({ cardData, vcfData, qrCodeValue, isExporting }, ref) => {
    ...
    const logoContainerClasses = cn(
      "relative w-12 h-12 md:w-20 md:h-20 shrink-0 flex items-center justify-center overflow-hidden",
      {
    ...
    useEffect(() => {
        const checkSize = () => {
            setQrCodeSize(window.innerWidth < 768 ? 80 : 120);
        };
        checkSize();
    ...
    const QrIcon = () => {
        ...
        return (
            <Icon className="h-5 w-5 md:h-7 md:h-7 text-[var(--card-primary-color)]" />
        );
    };
    ...
    return (
      ...
      >
        ...
        <div className={cn("relative z-10 w-[65%] h-full p-2 md:p-6 flex flex-col justify-between items-start text-left", (hasBgImage || !useWhiteBg) ? 'bg-card/80 backdrop-blur-sm' : 'bg-card')}>
          <header className="flex items-center gap-2 md:gap-4">
              ...
              <div className='flex-grow'>
                  ...
                  {(cardData.companySlogan || !cardData.companyName) && <p className="text-[10px] md:text-base mt-1 text-muted-foreground break-words">{cardData.companySlogan || "Your Company Slogan"}</p>}
              </div>
          </header>

          <main className='w-full'>
              <h1 className="text-base md:text-4xl font-bold text-[var(--card-primary-color)] break-words">{cardData.contactPersonName || "Your Name"}</h1>
              <p className="text-xs md:text-xl text-muted-foreground break-words">{cardData.designation || "Your Designation"}</p>
          </main>
          
          <footer className="w-full flex flex-col gap-1 text-[9px] md:text-sm">
            {cardData.contactDetails?.map((contact, index) =>
                (contact.phone || !cardData.contactPersonName) && (
                <div key={`phone-${index}`} className="flex items-center gap-1.5 md:gap-2 break-all">
                    <Phone className="h-3 w-3 md:h-4 md:w-4 shrink-0 text-muted-foreground" />
                    <span>{`${contact.name || 'Phone'}: ${contact.phone ? `${contact.countryCode || ''} ${contact.phone}`: 'xxxxx-xxxxx'}`}</span>
                </div>
                )
            )}
            {cardData.links.email.enabled && (cardData.links.email.value || !cardData.contactPersonName) && (
                <p className="flex items-center gap-1.5 md:gap-2 break-all"><Mail className="h-3 w-3 md:h-4 md:w-4 shrink-0 text-muted-foreground" /><span className="truncate">{cardData.links.email.value || "your.email@example.com"}</span></p>
            )}
            {cardData.links.website.enabled && (cardData.links.website.value || !cardData.contactPersonName) && (
                <p className="flex items-center gap-1.5 md:gap-2 break-all"><Globe className="h-3 w-3 md:h-4 md:w-4 shrink-0 text-muted-foreground" /><span className="truncate">{cardData.links.website.value || "your-website.com"}</span></p>
            )}
            {(cardData.address || !cardData.contactPersonName) && (
                <p className='flex items-start gap-1.5 md:gap-2 pt-1 break-words'><MapPin className="h-3 w-3 md:h-4 md:w-4 shrink-0 text-muted-foreground mt-0.5" /><span>{cardData.address || "123 Example St, City"}</span></p>
            )}
          </footer>
        </div>

        <div className={cn("relative z-10 w-[35%] h-full p-2 md:p-4 flex flex-col justify-center items-center text-center text-primary-foreground space-y-2 md:space-y-6", hasBgImage ? 'bg-primary/80 backdrop-blur-sm' : 'bg-[var(--card-primary-color)]')}>
            <div className="flex flex-col items-center gap-2">
                <div data-qr-code-wrapper="true" className="relative group/qr p-1.5 md:p-2 bg-white rounded-lg border-2 border-white">
                    {qrCodeValue ? (
                        <div className="relative" style={{ width: qrCodeSize, height: qrCodeSize }}>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                                <div className="h-6 w-6 md:h-10 md:w-10 bg-white/90 rounded-full flex items-center justify-center">
                                    <QrIcon />
                                </div>
                            </div>
                        </div>
            ...
            <div className="flex flex-wrap gap-2 md:gap-4 justify-center items-center">
              {activeLinks.map(([key, link]) => {
                  ...
                  if (!Icon) return null;
                      ...
                      if (['call', 'location', 'email', 'website'].includes(key)) return null;

                      if (key === 'workGallery') {
                      return showGallery && (
                          <React.Fragment key={key}>
                          {hasUploadedImages ? (
                              <Dialog><DialogTrigger asChild><button data-gallery-trigger="true" className={cn("group focus:outline-none", !hasValue && "pointer-events-none opacity-50")}><Icon className="h-4 w-4 md:h-7 md:h-7 text-primary-foreground transition-all duration-300 group-hover:text-white/80" /></button></DialogTrigger>
                          ...
                          ) : ( <a data-gallery-trigger="true" href={hasValue ? href : '#'} target={hasValue ? "_blank" : "_self"} rel="noopener noreferrer" className={cn("group", !hasValue && "pointer-events-none opacity-50")}><Icon className="h-4 w-4 md:h-7 md:h-7 text-primary-foreground transition-all duration-300 group-hover:text-white/80" /></a> )}
                          </React.Fragment>
                      )
                  }
                  if (key === 'brochure') {
                  ...
                  return(
                      <a key={key} href={hasValue ? href : '#'} target={hasValue ? "_blank" : "_self"} rel="noopener noreferrer" className={cn("group", !hasValue && "pointer-events-none opacity-50")}>
                          <Icon className="h-4 w-4 md:h-7 md:h-7 text-primary-foreground transition-all duration-300 group-hover:text-white/80" />
                      </a>
                  );
              })}
            </div>
